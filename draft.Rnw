...
require(reshape2)           ### For melt() before ggplotx
...
\author{Samuel Shleifer\footnote{R code available at
\url{github.com/sshleifer/ProsperThesis}}\\ Senior Essay in Economics}
\date{Yale University, \today}
\maketitle 
\begin{abstract}
This essay analyzes the effect of information gathering costs on financial market
efficiency, using data from Prosper.com, America's first Peer-to-Peer Lending Marketplace.
Peer-to-peer lending is the practice of individual and institutional investors
lending money to borrowers over the internet without the assistance of a
traditional financial intermediary. Between 2006 and 2011, Prosper underwent two
major policy changes that guided and then forced investors to set higher
interest rates, and reduce their exposure to subprime borrowers.

During the policy regime before either policy intervention (2006-2008), investors set
interest rates by auction, made avoidably bad mistakes, and earned poor returns.
The data from the first regime contradicts three Efficient Markets
Hypothesis predictions: that (1) risky assets should earn positive expected returns,
(2) riskier assets should earn higher returns than less risky assets, and, most
conclusively, (3) ``prices should fully reflect publicly available information."

Controlling for macroeconomic differences between the time periods of each policy regime
reveals that contradictions (1) and (2), which disappeared after Prosper began
guiding investors towards setting higher interest rates, are partly the
consequence of a turbulent macroeconomic period, but that better investor
decisions, or Prosper setting interest rates from the beginning, would have
weakened the blow of the financial crisis. Contradiction (3), that strategies
using publicly available information can earn superior risk-adjusted returns,
which holds until Prosper begins directly setting rates, suggesting that 
centralized interest rate setting may lead to a more efficient market than the
unconstrained interaction of supply and demand, especially when information
gathering costs are high.
\end{abstract}
\newpage

\section{Introduction}
Historically, there are two major perspectives of efficient market prices. Fama
...
A comparison of loan performance in different regimes allows me to consider two contrasting views of how an internet marketplace such as Prosper works.  One view, closely linked to Hayek, suggests that the marketplace would work best with minimal intervention from the market maker Prosper, so many individual lenders incorporate their private knowledge and information in the determination of the interest rate.   An opposing view would hold that individuals are not sophisticated or knowledgeable enough to assess loan risk, and some guidance or even interest rate setting from the market maker Prosper would improve loan performance.  The evidence on loan returns in the three regimes points quite clearly in favor of the market maker hypothesis, and against Hayek: as Prosper becomes more involved in the rate-setting process, the excess returns available using publicly available information approach 0, while aggregate investor returns exhibit higher mean and lower variance.
...
the policy changes can be accurately isolated by my methodologies.
The contradictions with efficiency hypotheses 1 and 2 can certainly be partially
...
by poor investor decisions, as evidenced by the predictable mispricings that can
be exploited, as they are in Section 5, the core of my market inefficiency
argument, which finds that over a somewhat longer period of time, simple loan
...
%if was all time, why would Prosper use same variables to make grades.
%Some of the evidence is inconsistent with time period argument. Can find variables, once Prosper starts using same variables, no more excess returns. Prosper themselves understood this.
...
In October 2008, Prosper was shut down by the SEC, and re-opened in July 2009
...
basis of a borrower's credit history.
...
In exchange, In exchange for verifying borrowers' identities and managing the
...
\iffalse
\startchronology
[startyear=2006,stopyear=2011,height=1ex,width=\hsize]
...
From April 19, 2006 until December 20, 2010. Prosper operated a \emph{variable
...
Unsuprisingly the automated feature selection procedure in section 5, which
...
variables for predicting default.
...
In the P2P lending context, this market price is the interest rate determined by
investors (or accepted by investors in the fixed-rate regime) at origination
time, and paid by the borrower, and returns are the cash flows received from borrowers divided by the amount invested.
...
different variables, and confirm the finding of negative expected returns in the
...
Instead of a focus on soft information, I describe the hard information that
...
Rather differently than Hayek theorizes, the data suggest that when investors are left on their own
to set prices, markets are not nearly as efficient as the prices set by ``a
single mind'', that aggregates the data. Furthermore, as interest rate setting
becomes more centralized, interest rates are more sensitive to the hard credit variables
that actually predict default.
%t and other unused citations can be found in hypo.cut.tex
...
As shown in Table \ref{sample}, this leaves us with many more early auction regime loans than the post shutdown regimes.\newline
<<sampTab, cache=FALSE>>=
...
performance, such as payments made, interest rate, and principal lost. Borrower
characteristics, including credit variables, which are usually obtained directly
...
repay average + 24\% ROI. Furthermore, my ROI results are very similar as Freedman
and Jin's, who use discounting.\footcite{freedman} %TODO(SS): CITE FED RATES

The rest of this paper documents my results, which are split into three sections.
...
across regimes. The section closes by examining the impact of Prosper's interventions, which reduce the valid borrower pool to a less-risky subset and assign borrowers more informative credit grades, on investor returns. 
...
These findings suggests that although investors were unlucky in
...
participants,without the meddling of a central planner.
...
<<roivtime,cache=FALSE,fig.height=3, fig.cap="This chart shows the mean ROI of loans originated each month in our sample. The blurred lines show the average ROI across all loans. The solid lins show is the average ROI for loans to borrowers in the 640-660 credit score bucket. Loans funded after the shutdown are associated with higher ROI, even controlling for borrower creditworthiness.">>=
ggplot() +
...
Figure \ref{fig:roivscore} shows that lower credit score loans earn lower returns in the early auction regime, inconsistent with the EMH prediction of a risk, reward tradeoff.
...
We now examine default rates and interest rates, the two features that explain the variation inreturns.
...
rate, not the chart itself} In the late auction regime, a 100 point increase in 
...
correlation between credit score and interest rate is -9.65\%, meaning borrowers
with lower credit scores paid higher interest rates. This pattern cannot be
...
regime investors' insensitivity to variance in credit score among risky
...
ggplot(d2, aes(CreditScoreEst,dr,color=regime))+
  geom_line(aes(size=num)) +
  geom_vline(xintercept=600,linetype=2) +
  xlim(460,NA) +
  scale_y_continuous(label=percent,limits=c(0,.8)) +
  theme_bw() +
  #scale_x_continuous(label=percent) +
  labs(x='Credit Score Bucket',y='Default Rate',size='Number of Loans',color='Regime')
@

<<defscore.tab,fig.height=3,cache=FALSE,fig.pos='H'>>=
...
However, much of this effect is caused by the fact that borrowers in the late auction regime were in higher credit buckets. Inside of each credit bucket, the difference between default rates is, on average, 10.2\%. Just as with interest rates, the gap is wider for risky borrowers, the difference in default rates for borrowers in buckets above 800 were 21\% more likely to default than borrowers in the late 

For credit buckets above 600, the relationship between credit score and default rate appears to be fairly linear, for all three regimes, suggesting that borrowers should be sensitive to credit scores when setting interest rates, since they are predictive of default.\footnote{I have a chart (suggested by Lapo) that shows what rate investors would have needed to charge to break even. It shows, as expected, that investors in early regime would have needed to charge much higher rates, especially at risky credit grades to break even. Should I put it in?}
Early auction regime investors, then, were not sufficiently sensitive to the hard credit variables that are predictive of default, especially when evaluating risky, low-credit borrowers, suggesting that the poor returns of the early auction regime were not caused only by the time period, but also by poor investor decisions. We now look at the more informative grading algorithm Prosper began using after the shutdown, which made investors more sensitive to hard credit variables.
...
When Prosper began aggregating hard credit variables into more informative grades, investors relying heavily on these grades when determining interest rates.
...
xtable(rf, label='ratefit',caption= 'This table documents how much of the variance in interest rates in each of the three regimes can be explained by variance in hard credit variables, including grades. \'Fit\' is measured as the Adjusted R squared of a regression with interest rate as the dependent variable and standard credit variables as independent variables, using every loan in the relevant regime.')
...
Figures \ref{fig:roivscore} \ref{defvscore} show that this sensitivity to credit
...
which variables had been past predictors of loan performance, as such, I do not
...
created by the solutions, of which there are thankfully few.
...
variables Prosper used to make its more informative grading, and that
...
The following procedure is executed on the training data for each of the three regimes to determine the 8 features\footnote{I use the terms 'feature' and 'Independent Variable' interchangeably.'} to use in the regression. It is automated by the leaps R package\footcite{leaps}, so I cannot choose features that I know to be powerful.
\begin{enumerate}
...
\item Choose the subset of features with the highest BIC. These 8 features will then be used in the training regressions.
\end{enumerate}
...
\item Start with 45 Features, select the 8 feature subset that best predicts Known-default.\footcite{leaps}
\item Run Linear Regression to predict Known-Default using training period data and selected variables.
...
xtable(fs3,label='aggstats',
       caption='This table compares the out of sample performance of the linear model (LM) strategies to the benchmark (BM). As shown in the next figure. The column names are in the format (Strategy).(statistic), where (LM) for linear model and (BM) for benchmark are the two strategies, and mean return of selected loan (mean), standard deviation of monthly returns (sd) and mean default rate of selected loans (default) are the statistics. For example, the LM.mean column shows the average ROI of a loan selected (out of sample, of course) by the linear model in each of the three regimes. The Excess column is calculated as LM.mean - BM.mean, and thus represents the difference in returns between the loans selected by the linear model and the benchmark over each regime. Notice that the Excess returns are positive for the first two regimes, where investors were involved in setting interest rates, and roughly 0 in the most recent regime, where Prosper set rates directly.', digits=3)
@
...
Empirical validation of this theory can be found in Wei and Lin (2013)'s
...
interest rates, they defaulted more frequently.\footcite{wei} With this in mind, my adjusted ROI serves as an \textbf{upper bound} for the possible ROI because it assumes that each borrower makes the same number of payments, even though each payment is higher. More realistically, we might expect Bill to make fewer payments on his loan if he found it difficult to afford his interest rate.
...
\subsection{Comparison to Traditional Loans (Unfinished)}
...
@
By comparing the performance of Prosper loans originated in each period to the performance of traditional consumer credit loans originated in each period, controlling for credit score as best we can\footnote{Method: Given the default rates of some consumer loans, and a table of the frequency of a bunch of credit scores, I created the same credit score groups in the Prosper data, took each group's mean default rate, and took the weighted average default rate for each year, with the group's representation in the Capital One borrower pool, shown in Table \ref{macro.samp} as the weights. Since the Capital One loans are of different maturities, I measured the portion of defaults within three years of origination.} gain a better understanding of the time period. The data, presented in Table \ref{macro} show that for all default rates are higher for Prosper loans than Capital One loans, but that the difference is much larger in the early auction regime. for the prosper data than the Capital One data, but the difference is most extreme for the early auction regime. There may again be flawed assumptions, including adverse selection of borrowers on the internet, other credit variables besides score, and the different maturities of the loans, but the data again suggest that early auction regime investors did a particularly bad job.\footnote{This may be in the right place. I spent a while looking for the ideal data on the web and this is the closest I came.}

\section{Conclusion (TODO)}
\begin{enumerate}
\item Restate results.
\item Central planner intervention may be useful when info gathering costs are high.
\item Future research: same experiment with known ROI. Better time controls? Look at consistent errors in mistakes borrowers made.
\end{enumerate}
\newpage
%\begin{appendices}
\section{Appendix}
...
@
\subsection{Rate Adjustment Results, by Credit Grade}
...
prate = filter(decomp, regime=='Auction-Late')
cs = inner_join(auc,adj, by='CreditScoreEst') %>% inner_join(prate,by='CreditScoreEst')
