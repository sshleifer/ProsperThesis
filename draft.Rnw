...
%\linespread{1.3}
\title{YALE DEPARTMENT OF ECONOMICS \\ \vspace*{60pt} The Folly of the Crowd: \\ Market Inefficiencies in Peer to Peer Lending}
\author{Samuel Shleifer\footnote{R code available at
\url{github.com/sshleifer/ProsperThesis} TODO(THANKS(VK, Pope, AS, Nick, Iyer, Metrick)}}
\date{\today}
\maketitle
\begin{center}
...
major policy changes that guided and then forced investors to lend at higher
...
The data from this first regime is inconsistent with three of the predictions of
the Efficient Markets Hypothesis predictions: that (1) risky assets should earn positive expected returns,
...
reveals that inconsistencies (1) and (2), which disappeared after Prosper began
guiding investors towards setting higher interest rates, are partly the
consequence of a turbulent macroeconomic period. Inconsistency (3), that strategies
using publicly available information can earn superior risk-adjusted returns,
which holds until Prosper begins directly setting rates, suggests that 
centralized interest rate setting by a market maker may lead to a more efficient market than the
...
In the P2P Lending context, the market's price is the interest rate charged on a loan. This paper studies the question of whether interest rates on Prosper.com are set efficiently. (Fama, 1970)'s semi-strong EMH predicts that if interest rates were set efficiently, then...
\begin{enumerate}
...
risk, and some guidance or even interest rate setting from the market maker
Prosper would improve loan performance. The evidence on loan returns in the
three regimes points quite clearly in favor of the market maker hypothesis,
and against Hayek: as Prosper becomes more involved in the rate-setting process,
...
reutrns. In the Prosper-rate regime, when Prosper sets interest rates directly,
they cannot. The two periods are very similar macroeconomically. Perhaps most tellingly,
in the period when Prsper sets rates it seems to use precisely the information that was
helpful in selecting the best performing loans in the early auction regime.
The points strongly toward the conclusion that initial investors were not using
...
Unsuprisingly, buut critically for the interpretation of the evidence,
...
Under the current \emph{Prosper-rate} model, enacted December 21, 2010, lenders
...
field" of the application, and find that descriptions that are easy to read have
2.3\% lower default rates than average, while complex narratives are associated
with a 3.6\% higher default rate, but do not analyze return on
investment.\footcite{gao} Pope and Sydnor (2008) find that investors can reduce
their defaults by statistically discriminating against racial minorities, but do
not analyze return on investment. Iyer et. al (2009) find that within the credit score buckets provided by Prosper, investors use soft information to further sort borrowers within buckets, reducing defaults.\footcite{iyer}
...
single mind'', namely a market maker who aggregates the data. Furthermore, as interest rate setting
...
\end{equation}
Where amount.repaid is the sum of the borrower's monthly payments. As shown in
...
<<ratevscore, cache=FALSE, fig.cap='This chart shows the median interest rate borrowers from each credit score bucket paid during each period. Under the early auction regime, interest rates were lower, especially for risky borrowers.',fig.height=6>>=
...
\subsection{ADDED}
The finding that the features that are selected to predict superior returns
...
that investors failed to attend to these variables, as Freedman and Jin suggest,
...
In summary, over the inital two regimes where investors were involved in the
...
are as compared to other Prosper loans, not other asset classes like
treasuries.} In the final regime, where the market maker sets the interest rate,
...
quickly, and intervened in completely decentralized price formation. Guidance,
and eventually price intervention from the market maker, likely improved returns
substantially. The evidence cautions against the assumption of universal
investor rationality. More important, it suggests that design of markets that
protects investors from their errors and limited attention, either by fairly
aggressive guidance or even price setting, can improve market performance
dramatically.

\begin{enumerate}
\item Restate results.
\item Market maker intervention may be useful when info gathering costs are high.
\item Platform invest in educating investors? (VK idea)
...
rateregs= list(lm(LenderYield ~ CreditScoreEst + CurrentDelinquencies + grade, data=o1),
             ...
             lm(LenderYield ~ CreditScoreEst + CurrentDelinquencies, data=o1),
             lm(LenderYield ~ CreditScoreEst + CurrentDelinquencies, data=o2),
             lm(LenderYield ~ CreditScoreEst + CurrentDelinquencies, data=n))
