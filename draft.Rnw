...
\author{Samuel Shleifer
  \footnote{Thanks to Professor Vineet Kumar for his help
  in developing the ideas and statistical methodologies involved in this paper,
  to Devin Pope and Rajkamal Iyer for help manipulating the Prosper data, 
...
\item Late Auction Regime: (July 20, 2009-December 19, 2010): Rates are set by investor auction, with Prosper offering valuable guidance in the form of more complex credit grading.
\item Prosper-rate Regime: (December 20, 2010 - August 3, 2011): Prosper sets rates using its more complex credit grading algorithm.
\end{enumerate}
...
in the period when Prosper sets rates it seems to use precisely the information that was
helpful in selecting the best performing loans in the early auction regime.
The points strongly toward the conclusion that early investors were not using
...
<<roivstatus,cache=FALSE,fig.height=3,fig.cap="This chart shows the distribution of ROI for completed and defaulted loans over all three regimes. A continuous ROI variable offers more information about the quality of an investment than a binary default indicator. Dotted lines indicate mean ROI for category.",fig.height=3>>=
...
auction regime.\footcite{fred} \footnote{The fed-funds rate's peak during our sample, is at 5\%, during the Early auction regime. It then falls to near 0 for the duration of the later two
...
As the quantity of publicly available information increases, crowds will utilize an increasingly small fraction of it, leading either to inefficient prices or, perhaps, a new trend of "re-intermediation."
\newpage
\section{Appendix}
