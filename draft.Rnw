...
The early auction regime is closest to Hayek's vision of the free market, since
interest rates are determined freely by investors through auctions, with few
limitations on which borrowers they can invest in.
I find that it is precisely in this free-market regime that the three
predictions of the EMH are contradicted.
In the two later regimes, when Prosper first provides much more guidance to investors, and then actually sets interest rates, investor returns improve dramatically.
The evidence across regimes is broadly inconsistent with the view that local knowledge and the wisdom of the crowd improves price discovery.
To the contrary, it appears that a "centralized' Prosper mechanism of either guiding or setting interest rates actually leads to fewer predictable mispricings: as Prosper becomes more involved in the rate-setting process, the excess returns available using publicly available information approach 0, while aggregate returns exhibit higher mean and lower variance.
...
variance of their returns. Before a potential borrower's listing is posted on
the website, Prosper authenticates their social security number, driver's
...
average, Lenders would have expected an internal rate of return of -.62\% to
...
Rather differently than Hayek, I find that when investors are left on their own
...
and Jin's, who use discounting.\footcite{freedman}
...
These findings suggests that although investors were extraordinarily unlucky in
