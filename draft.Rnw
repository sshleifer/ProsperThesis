...
This essay expands on the work of Freedman and Jin (2008), who use investor
learning to explain the improvement, with the benefit of three more years of
data and two major policy changes. 
Instead of investor learning as the catalyst that led to a more efficient market, I frame Prosper's rule changes as catalysts that guide
and then force investors towards making better decisions, and find that when
investors set rates by auction, investment strategies using publicly available
information can earn excess returns, an inconsistency with the predictions of
the Efficient Markets Hypothesis, but that once Prosper begins setting rates,
